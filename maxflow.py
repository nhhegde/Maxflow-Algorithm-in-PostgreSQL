...
def maxflow(bfs_max_iterations=float('inf'), flow_max_iterations=float('inf')):
    ...
    while True:
        ...
        while not sink_path_found:
            ...
            db.execute("""
                    SELECT array_cat(p.path, ARRAY[e1.id]) AS path, array_cat(p.nodes, ARRAY[e1.dst]) AS nodes
                    INTO tmp
                    FROM paths p, edge e1
                    WHERE NOT p.nodes @> ARRAY[e1.dst]
                    AND p.nodes[array_length(p.nodes, 1)] = e1.src
                    AND e1.capacity > 0;
        ...
        db.execute("""
            ...
            constraining_capacity(capacity) AS (
                SELECT MIN(e1.capacity)
                FROM path_edges pe, edge e1
                WHERE pe.path_edge=e1.id
            )
        ...
        db.execute("""
            WITH updates(id, new_capacity) AS (
                SELECT ftr.edge_id AS id, (e1.capacity - ftr.flow) AS new_capacity
                FROM flow_to_route ftr, edge e1
                WHERE ftr.edge_id=e1.id
                UNION
                SELECT fe.reverse_id AS id, e1.capacity + ftr.flow AS new_capacity
                FROM flow_to_route ftr, flip_edge fe, edge e1
                WHERE ftr.edge_id=fe.forward_id
                AND fe.reverse_id = e1.id
            )
            UPDATE edge
